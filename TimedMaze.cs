...
public class TimedMaze : MonoBehaviour {
	[System.Serializable]
	public class Cell {
	...
	// New variables added for the time constraints below
	...
	public static TimedMaze control;
	...
	void Update () {
		...
		if (isRunning) {
			...
			score.GetComponent<Text> ().text = "You have completed " + levelsFinished.ToString () + " levels";
			if (timeLeft <= 0) {
				GameOver ();
				isRunning = false;
				intTime = 0;
